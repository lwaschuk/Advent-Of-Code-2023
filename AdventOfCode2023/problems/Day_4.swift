...
class Day_4 {
  ...
  internal class Helper {
    func deserialize(_ fullCard: String) -> (gameNumber: Int, score: Int) {
      ...
      let cardNumber = Int(temp[0].replacingOccurrences(of: "Card", with: "").filter{!$0.isWhitespace}.replacingOccurrences(of: ":", with: "")) ?? -1
      let winningNumbers: [Int : Int] = values[0].split(separator: " ").reduce(into: [:]) { (result, value) in
