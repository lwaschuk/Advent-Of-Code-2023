...
class Day_5 {
  ...
  func getLocations(seedRange: RangeSet<Int>? = nil, seedList: [Int]? = nil) -> [Int] {
    ...
    var fertilizerToWaterMapping = helper.extractMapping(from: input, for: "fertilizer-to-water map:")
    var waterToLightMapping = helper.extractMapping(from: input, for: "water-to-light map:")
    var lightToTemperatureMapping = helper.extractMapping(from: input, for: "light-to-temperature map:")
    var temperatureToHumidityMapping = helper.extractMapping(from: input, for: "temperature-to-humidity map:")
    var humidityToLocationMapping = helper.extractMapping(from: input, for: "humidity-to-location map:")
...
internal class Helper {
  ...
  func getDistination(for source: Int, in map: inout [Int: (destinationStart: Int, offset: Int)]) -> Int {
